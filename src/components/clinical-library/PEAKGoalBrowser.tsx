...
import { Search, ChevronRight, Brain, Target, Layers } from 'lucide-react';
...
export function PEAKGoalBrowser({ clientId }: Props) {
  const { data: domains = [] } = useLibraryDomains('peak');
  const { data: subdomains = [] } = useLibrarySubdomains('peak');
  const { data: goals = [], isLoading } = useLibraryGoals('peak');
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h3 className="text-sm font-bold">PEAK Relational Training Goals</h3>
        <Badge variant="outline" className="text-[9px]">curriculum library</Badge>
      ...
      {isLoading ? (
        <p className="text-xs text-muted-foreground py-6 text-center">Loading PEAK goals…</p>
      ) : filtered.length === 0 ? (
        <Card>
          <CardContent className="py-8 text-center">
            <Target className="w-6 h-6 mx-auto text-muted-foreground/40 mb-2" />
            <p className="text-xs text-muted-foreground">No PEAK goals found matching filters.</p>
          </CardContent>
        </Card>
      ) : (
