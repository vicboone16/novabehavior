...
import { Search, ChevronRight, Baby, Target, Layers } from 'lucide-react';
...
export function ESDMGoalBrowser({ clientId }: Props) {
  const { data: domains = [] } = useLibraryDomains('esdm');
  const { data: subdomains = [] } = useLibrarySubdomains('esdm');
  const { data: goals = [], isLoading } = useLibraryGoals('esdm');
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h3 className="text-sm font-bold">ESDM Developmental Goals</h3>
      ...
      {isLoading ? (
        <p className="text-xs text-muted-foreground py-6 text-center">Loading ESDM goals…</p>
      ) : filtered.length === 0 ? (
        <Card>
          <CardContent className="py-8 text-center">
            <Target className="w-6 h-6 mx-auto text-muted-foreground/40 mb-2" />
            <p className="text-xs text-muted-foreground">No ESDM goals found matching filters.</p>
          </CardContent>
        </Card>
      ) : (
