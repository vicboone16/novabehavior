...
import { Search, ChevronRight, BookOpen, Target, Layers, Plus } from 'lucide-react';
...
export function AFLSCurriculumBrowser({ clientId }: Props) {
  const { data: domains = [] } = useLibraryDomains('afls');
  const { data: subdomains = [] } = useLibrarySubdomains('afls');
  const { data: goals = [], isLoading } = useLibraryGoals('afls');
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h3 className="text-sm font-bold">AFLS Curriculum Goals</h3>
      ...
      {isLoading ? (
        <p className="text-xs text-muted-foreground py-6 text-center">Loading AFLS goals…</p>
      ) : filtered.length === 0 ? (
        <Card>
          <CardContent className="py-8 text-center">
            <Target className="w-6 h-6 mx-auto text-muted-foreground/40 mb-2" />
            <p className="text-xs text-muted-foreground">No AFLS goals found matching filters.</p>
          </CardContent>
        </Card>
      ) : (
