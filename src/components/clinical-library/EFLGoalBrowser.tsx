...
import { Search, ChevronRight, Shield, Target, Layers, Filter } from 'lucide-react';
...
export function EFLGoalBrowser({ clientId }: Props) {
  const { data: domains = [] } = useLibraryDomains('efl');
  const { data: subdomains = [] } = useLibrarySubdomains('efl');
  const { data: goals = [], isLoading } = useLibraryGoals('efl');
  ...
  if (selectedGoal) {
    return (
      <LibraryGoalDetail
        goal={selectedGoal}
        libraryName="EFL (Essential for Living)"
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h3 className="text-sm font-bold">EFL Curriculum Goals</h3>
        <Badge variant="outline" className="text-[9px]">curriculum</Badge>
      ...
      {isLoading ? (
        <p className="text-xs text-muted-foreground py-6 text-center">Loading EFL goals…</p>
      ) : filtered.length === 0 ? (
        <Card>
          <CardContent className="py-8 text-center">
            <Target className="w-6 h-6 mx-auto text-muted-foreground/40 mb-2" />
            <p className="text-xs text-muted-foreground">No EFL goals found matching filters.</p>
          </CardContent>
        </Card>
      ) : (
                ...
                <Card className="cursor-pointer hover:bg-muted/30 transition-colors">
                  <CardContent className="p-2.5 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-semibold">{domainName(dk)}</span>
                        ...
                        >
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex gap-0.5 mt-0.5 flex-wrap">
                              {goal.goal_type && (
                                <Badge variant="outline" className="text-[7px]">{goal.goal_type}</Badge>
                              )}
                              {(goal.tags ?? []).slice(0, 3).map(t => (
                                <Badge key={t} variant="secondary" className="text-[7px]">{t}</Badge>
                              ))}
                            </div>
                          </div>
