              ...
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-xs font-semibold truncate">{g.target_name || g.goal_title || 'Goal'}</span>
                    {g.mastery_status && (
                  ...
                  <div className="mt-2 pt-2 border-t border-border/50">
                    <TreatmentIntelligenceActions
                      ...
                      title={`Goal: ${g.target_name || g.goal_title || 'Untitled'}`}
                      compact={false}
                    />
                  </div>
                </div>
                <div className="flex flex-col gap-1 shrink-0">
                  {onAddTalkingPoint && (
                    <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => onAddTalkingPoint('goals', `${g.target_name || g.goal_title || 'Goal'}: ${pct}% to mastery, status: ${g.mastery_status}`)}>
