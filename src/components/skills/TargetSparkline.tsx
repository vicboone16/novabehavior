...
export function TargetSparkline({ targetId }: TargetSparklineProps) {
  ...
  if (!sparklineData.length || sparklineData.length < 2) {
    return <span className="text-[10px] text-muted-foreground/50 italic">No data</span>;
  }
    ...
    <span className="inline-flex items-center gap-1">
      <svg width={100} height={28} className="shrink-0">
        ...
        {(() => {
          ...
          return <circle cx={parseFloat(last[0])} cy={parseFloat(last[1])} r={2.5} fill={color} />;
        })()}
      </svg>
      <span className="text-[10px] font-medium" style={{ color }}>{lastVal}%</span>
    </span>
  );
}
