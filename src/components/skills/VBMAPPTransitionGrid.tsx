...
export function VBMAPPTransitionGrid({ studentId, studentName, assessment, onBack, onSave }: VBMAPPTransitionGridProps) {
  ...
  if (loading) {
    return <div className="text-center py-8 text-muted-foreground">Loading Transition Assessment...</div>;
  }
                    ...
                    <div className="flex flex-col-reverse gap-0.5 h-32">
                      {[1, 2, 3, 4, 5].map(level => (
                        <button
                          ...
                          onClick={() => handleScoreChange(item.id, currentScore === level ? level - 1 : level)}
                        >
                          {level}
                        </button>
                      ))}
