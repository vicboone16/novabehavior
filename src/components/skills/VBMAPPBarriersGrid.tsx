...
// Embedded mode props - used when rendered inside VBMAPPGrid
export interface EmbeddedBarriersGridProps {
...
/** Embedded barriers grid - no header, save, or back. Parent manages state. */
export function EmbeddedBarriersGrid({ curriculumSystemId, scores, onScoreChange }: EmbeddedBarriersGridProps) {
  ...
  if (loading) return <div className="text-center py-8 text-muted-foreground">Loading Barriers Assessment...</div>;
              ...
              <CardHeader className="py-3">
                <CardTitle className="text-sm flex items-center gap-2">
                  ...
                  {currentScore !== undefined && currentScore !== null && (
                    <Badge className={`ml-auto ${SCORE_COLORS[currentScore]}`}>Score: {currentScore}</Badge>
                  )}
                </CardTitle>
              </CardHeader>
                  ...
                  {[0, 1, 2, 3, 4].map(score => (
                    <Button
                      ...
                      className={`w-10 h-10 p-0 text-base font-bold ${
                        currentScore === score ? SCORE_COLORS[score] + ' ring-2 ring-offset-1 ring-primary' : ''
                      }`}
...
/** Standalone barriers grid - used when opened as its own assessment */
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              <span>•</span>
              <span>{scoredCount} / {items.length} scored</span>
            </div>
          </div>
        </div>
