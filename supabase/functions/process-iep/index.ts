...
/**
 * IEP Document Processing Pipeline (Nova Core)
 * 
 * Steps:
 ...
 * 8. validate_goals — Run quality & compliance checks
 */
...
serve(async (req) => {
  ...
  try {
    ...
    // Use service role for DB writes (caller may be from satellite app with anon key)
    const supabase = createClient(supabaseUrl, serviceRoleKey, {
      auth: { autoRefreshToken: false, persistSession: false },
    });

    // Verify the caller has a valid token or is using the anon key
    const anonKey = Deno.env.get("SUPABASE_ANON_KEY")!;
    const token = authHeader.replace("Bearer ", "");
    
    // Try to get user from token; allow anon key callers (satellite apps)
    ...
    switch (step) {
      case "ocr_clean": {
        ...
        result = await callAI(GLOBAL_SYSTEM,
          `You are an expert document reconstruction engine specializing in IEPs (Individualized Education Programs).
