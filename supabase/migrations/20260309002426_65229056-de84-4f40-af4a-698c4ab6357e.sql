...
drop view if exists public.v_parent_training_progress_report_summary cascade;
drop view if exists public.v_parent_training_goal_engine_summary cascade;

-- Recreate goal engine summary view
...
-- Recreate progress report summary using module_completion_summary (which has caregiver_id)
create view public.v_parent_training_progress_report_summary as
select
    a.client_id,
    a.caregiver_id,
    count(distinct a.module_assignment_id) as assigned_module_count,
    count(distinct a.module_assignment_id) filter (where lower(coalesce(a.status,'')) = 'completed') as completed_module_count,
...
from public.v_parent_training_module_completion_summary a
left join public.v_parent_training_goal_engine_summary g
  on g.client_id = a.client_id and g.caregiver_id = a.caregiver_id
left join public.v_parent_training_homework_summary h
  on h.client_id = a.client_id and h.caregiver_id = a.caregiver_id
left join public.parent_training_session_logs s
  on s.client_id = a.client_id and s.caregiver_id = a.caregiver_id
group by a.client_id, a.caregiver_id;

-- Promote custom goal to library
...
begin
    select * into v_custom from public.parent_training_custom_goals where id = p_custom_goal_id;
    if not found then raise exception 'Custom goal not found'; end if;

    insert into public.parent_training_goals (
        module_id, goal_key, title, description, goal_title, goal_description,
        measurement_method, baseline_definition, target_definition, mastery_criteria, unit, is_active
    ) values (
        v_custom.module_id, p_goal_key, v_custom.goal_title, v_custom.goal_description,
        v_custom.goal_title, v_custom.goal_description, v_custom.measurement_method,
        v_custom.baseline_definition, v_custom.target_definition, v_custom.mastery_criteria,
        v_custom.unit, true
    ) returning id into v_library_goal_id;

    update public.parent_training_custom_goals
    set promoted_goal_id = v_library_goal_id, add_to_library = true, updated_at = now()
...
-- Log goal data with auto-calculation
...
begin
    insert into public.parent_training_data (
        goal_assignment_id, caregiver_id, data_value, data_text, notes, entered_by
    ) values (
        p_goal_assignment_id, p_caregiver_id, p_data_value, p_data_text, p_notes, p_entered_by
    ) returning id into v_data_id;

    select baseline_value, target_value into v_baseline, v_target
    from public.parent_training_goal_assignments where id = p_goal_assignment_id;
    ...
    if p_data_value is not null and v_target is not null then
        ...
        if p_data_value >= v_target then v_status := 'mastered';
        elsif p_data_value > coalesce(v_baseline, 0) then v_status := 'in_progress';
    ...
    set current_value = coalesce(p_data_value, current_value),
...
begin
    select coalesce(jsonb_agg(jsonb_build_object(
        'module_assignment_id', a.id, 'module_key', m.module_key,
        'module_title', m.title, 'status', a.status,
        'assigned_at', a.assigned_at, 'due_date', a.due_date
    ) order by m.order_index), '[]'::jsonb)
    into v_modules
    from public.parent_training_assignments a
    join public.parent_training_modules m on m.id = a.module_id
    where a.client_id = p_client_id and a.caregiver_id = p_caregiver_id;
    ...
    where eg.client_id = p_client_id and eg.caregiver_id = p_caregiver_id;

    select coalesce(jsonb_agg(jsonb_build_object(
        'session_date', l.session_date, 'service_code', l.service_code,
        'duration_minutes', l.duration_minutes, 'session_summary', l.session_summary,
        'caregiver_response', l.caregiver_response, 'homework_assigned', l.homework_assigned,
        'next_steps', l.next_steps
    ) order by l.session_date desc), '[]'::jsonb)
    into v_sessions
    from public.parent_training_session_logs l
    where l.client_id = p_client_id and l.caregiver_id = p_caregiver_id;

    return jsonb_build_object(
        'client_id', p_client_id, 'caregiver_id', p_caregiver_id,
        'modules', v_modules, 'goals', v_goals, 'session_logs', v_sessions
    );
end;
$$;
